...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Complex
{
    public class SpinningBladePillar : Template
    {
        ...
        {
            builder.Add(Guid.Parse("202a3139-430a-4841-8488-9eefbe0089fe"), ActionTypes.Instances.Reaction.ID, "Rising Pillar", action =>
            {
                action
                    .Trigger("A creature steps on one of the trapped floor tiles.")
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("8c6ac794-a869-4f98-8fda-c9157022eb44"), "The trap pops up in a grid intersection and makes a spinning blade Strike against one adjacent creature (if any), then rolls initiative.");
                    });
            });
        }
