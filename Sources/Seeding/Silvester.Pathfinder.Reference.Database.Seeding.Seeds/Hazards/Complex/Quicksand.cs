...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Complex
{
    public class Quicksand : Template
    {
        ...
        {
            builder.Add(Guid.Parse("d1adf885-1c78-4d30-b759-12798dc37a4a"), ActionTypes.Instances.FreeAction.ID, "Submerge", action =>
            {
                action
                    .Trigger("A Huge or smaller creature walks onto the quicksand.")
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("a9f37fc2-82d6-466c-adc8-552f4b20da56"), "The triggering creature sinks into the quicksand up to its waist. The quicksand rolls initiative if it hasn't already.");
                    });
            });
        }
