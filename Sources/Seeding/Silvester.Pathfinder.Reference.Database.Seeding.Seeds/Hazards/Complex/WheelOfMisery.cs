...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Complex
{
    public class WheelOfMisery : Template
    {
        ...
        {
            builder.Add(Guid.Parse("29a2a4fe-936b-4b1a-8aa0-bd1e6ca3fc88"), ActionTypes.Instances.Reaction.ID, "Wheel Spin", action =>
            {
                action
                    .Trigger("A creature enters the sensor's detection area.")
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("78b61a1b-9b9c-4637-a632-d5cac2d621b6"), "The wheel begins to spin and rolls initiative.");
                    });
            });
        }
