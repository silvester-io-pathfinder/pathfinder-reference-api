...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Simple
{
    public class SlammingDoor : Template
    {
        ...
        {
            builder.Add(Guid.Parse("57f67171-9fe7-42a8-9338-9dfcff0c17b1"), ActionTypes.Instances.Reaction.ID, "Slam Shut", action =>
            {
                action
                    .Trigger("Pressure is placed on any floor tile.")
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("67f6c155-f410-4fdb-bfdf-ea6988d88003"), "The door falls, closing off the hallway. The stone slab deals 3d8 bludgeoning damage to anyone beneath or adjacent to the slab when it drops and pushes them out of its space in a random direction. A creature that succeeds at a DC 17 Reflex save takes no damage and rolls out of the way in a random direction. On a critical success, they can choose the direction.");
                        details.Text(Guid.Parse("85896c21-0853-44e8-90ea-b304cdbb0f14"), "Lifting the fallen slab requires a successful DC 25 Athletics check. Hitting the floor panels triggers the trap. The slab uses the same AC and saves as the trap, but it has Hardness 12, HP 48 (BT 24).");
                    });
            });
        }
