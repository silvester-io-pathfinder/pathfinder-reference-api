...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Simple
{
    public class BottomlessPit : Template
    {
        ...
        {
            builder.Add(Guid.Parse("54b9c0cb-563f-412d-aa62-829de3b015c5"), ActionTypes.Instances.Reaction.ID, "Infinite Pitfall", action =>
            {
                action
                    .Trigger("A creature walks onto the trapdoor.")
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("63bd40ef-eda7-4641-8d14-e4cd5efbfbaf"), "The triggering creature falls in and continues to fall, potentially forever. That creature can try to Grab an Edge to avoid falling (page 472). The DC to Climb the walls or Grab an Edge is 26");
                        details.Text(Guid.Parse("3b3bacc8-6bac-46c7-afff-17183a183c5a"), "The pit contains many handholds, so the falling creature can try to Grab an Edge again every 6 seconds. If the creature succeeds, it can start to Climb out from that point (though it might be a very long climb, depending on how far the creature fell). Since the creature falls endlessly, it can rest and even prepare spells while falling, though items dropped while falling are usually lost forever.");
                    });
            });
        }
