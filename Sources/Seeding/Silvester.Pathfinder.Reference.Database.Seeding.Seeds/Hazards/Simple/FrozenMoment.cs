...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Simple
{
    public class FrozenMoment : Template
    {
        ...
        {
            builder.Add(Guid.Parse("21a56455-6731-4be9-bc6b-02130bac5957"), ActionTypes.Instances.Reaction.ID, "Adrift in Time", action =>
            {
                action
                    .Trigger("A creature touches the warded object or area/")
                    .Traits(traits =>
                    {
                        traits.Add(Guid.Parse("216dc911-ca64-4d55-bf36-854636345ca3"), Traits.Instances.Occult.ID);
                        traits.Add(Guid.Parse("3bc0275d-786e-4012-b501-63b4c9772572"), Traits.Instances.Transmutation.ID);
                    })
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("806735c8-382d-4647-a8fd-b33e379cebad"), "The triggering creature and all creatures within 30 feet are trapped in a disrupted time flow (DC 38 Fortitude negates). The creatures' minds move so quickly that each round seems to last a century, but their bodies and magical energies move so slowly that they can't use any actions except Recall Knowledge. An affected creature must attempt a DC 36 Will saving throw against a warp mind spell immediately and again for every minute of real time that passes while the creature is trapped in the frozen moment. This effect has an unlimited duration but can be counteracted.");
                    });
            });
        }
