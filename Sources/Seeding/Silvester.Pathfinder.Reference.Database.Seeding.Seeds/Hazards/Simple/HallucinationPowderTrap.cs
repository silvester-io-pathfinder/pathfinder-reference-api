...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Hazards.Simple
{
    public class HallucinationPowderTrap : Template
    {
        ...
        {
            builder.Add(Guid.Parse("3df56b95-79f0-4ed7-bdb0-d0d0bdddcde2"), ActionTypes.Instances.Reaction.ID, "Powder Burst", action =>
            {
                action
                    ...
                    .Traits(traits =>
                    {
                        traits.Add(Guid.Parse("afab3c8c-5381-4ca0-9eea-ea8478c2588c"), Traits.Instances.Mental.ID);
                        traits.Add(Guid.Parse("dd5aee4f-0d22-4c60-9979-7ed3997a8e5b"), Traits.Instances.Poison.ID);
                    })
                    .Details(details =>
                    {
                        details.Text(Guid.Parse("06f5fbcf-7e70-4444-8f61-60927a67cef8"), "The tube explodes, spraying hallucinogenic powder in a 30-foot cone. Any creature in the cone must succeed at a DC 24 Will save or be confused for 1 round and take a –2 status penalty to Perception checks and saves against mental effects for 1d4 hours. On a critical failure, the penalty is instead –4.");
                    });
            });
        }
