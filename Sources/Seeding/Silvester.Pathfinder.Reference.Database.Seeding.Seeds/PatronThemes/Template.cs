...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.PatronThemes
{
    ...
    {
        protected override PatronTheme GetEntity(ISeedBuilder builder)
        {
            PatronTheme theme = GetPatronTheme();

            builder.AddSourcePage(theme, GetSourcePage(), (e => e.SourcePageId));
            builder.AddEffect(theme, GetEffects, e => e.EffectId);
            builder.AddJoinData<PatronTheme, Spell>(theme, GetGrantedSpellOptions());
