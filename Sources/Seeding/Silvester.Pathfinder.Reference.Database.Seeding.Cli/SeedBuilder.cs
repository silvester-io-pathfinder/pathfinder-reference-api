...
namespace Silvester.Pathfinder.Reference.Database.Seeding
{
    ...
    public class SeedBuilder : ISeedBuilder
    {
        ...
        private static ISkipNavigation GetFirstToSecondNavigation<TFirst, TSecond>(IEntityType firstEntityType, IEntityType secondEntityType)
        {
            ...
            {
                System.Console.WriteLine("Following navigational properties could not be found. Are you sure they're present and PROPERTIES (as opposed to FIELDS)?");
                System.Console.WriteLine($"First: {typeof(TFirst).Name}");
                System.Console.WriteLine($"Second: {typeof(TSecond).Name}");
                System.Console.WriteLine($"First Count: {skipNavigations1.Length} with entries '{string.Join(", ", skipNavigations1.Select(e => e.Name))}'.");
                System.Console.WriteLine($"Second Count: {skipNavigations2.Length} with entries '{string.Join(", ", skipNavigations2.Select(e => e.Name))}'.");
