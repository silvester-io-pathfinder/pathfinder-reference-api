...
namespace Silvester.Pathfinder.Reference.Database.Seeding.Seeds.Ancestries.Instances
{
    public class Orc : Template
    {
        ...
        protected override IEnumerable<TextBlock> GetDescription()
        {
            ...
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "If you want a character who is hardy, fearsome, and excels at feats of physical prowess, you should play an orc." };
        }
        ...
        {
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Heading, Text = "Orc History" };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Originating deep underground, orcs were driven upward when the dwarves began their Quest for Sky. Endless dwarf forces advanced from below, crossing into orc territory and inciting a war that lasted for countless orc lifetimes, with each new generation losing more ground. When finally driven to the surface, orcs discovered a world shrouded by ash. The nearby human civilizations were unprepared for their anger and desperation as they battled their way to empire with dauntless ferocity." };

            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Heading, Text = "Orc Holds" };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Orc holds are large groups of orcs bound together by family, values, and loyalty." };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Burning Sun: Armed with their faith in Sarenrae, the Burning Sun hold is dedicated to directing orc culture toward a more righteous path." };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Empty Hand: The Empty Hand hold collects taxes from trade in Urgir to acquire top-quality weapons and armor. The hold's renown reached new heights after they led a recent victory over the Whispering Tyrant's undead forces." };

            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Heading, Text = "Orc Settlements" };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Orc settlements rapidly rise and fall, but a few have stood the test of time." };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Garaguum: In the Darklands, orcs inhabit a ruin built by their ancestors: the final battleground of their people's last stand against the dwarf invaders." };
            yield return new TextBlock { Id = Guid.Parse(""), Type = TextBlockType.Text, Text = "Scarwall: When the curse lifted from this haunted castle within the Hold of Belkzen, the local orc holds rushed to make their claim. The castle's lingering magic warped the minds and bodies of the orcs, who now call themselves the Spirit Skull Hold." };
        }
