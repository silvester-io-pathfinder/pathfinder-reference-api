...
namespace Silvester.Pathfinder.Official.Database.Seeding.Seeds.Feats
{
    public class FeatSeeder
    {
        ...
        public FeatSeeder(ModelBuilder builder, Trait[] traits, ActionType[] actionTypes, FeatType[] featTypes, Proficiency[] proficiencies, Lore[] lores, HeritageRarity[] heritageRarities, ItemCategory[] itemCategories, Skill[] skills, Stat[] stats)
        {
            Builder = builder;
            Traits = traits;
        ...
        public void Seed()
        {
            foreach (Type templateType in GetType().Assembly.GetTypes().Where(e => e.IsSubclassOf(typeof(AbstractFeatTemplate))))
            {
                AbstractFeatTemplate template = (AbstractFeatTemplate) Activator.CreateInstance(templateType)!;
                template.Seed(this);
            }
        }
